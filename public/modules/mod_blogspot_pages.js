export async function mount(ctx){
  ...
  host.innerHTML = `
    <div class="space-y-4">
      <div>
        <div class="text-sm font-bold">Static Pages</div>
        <div class="text-xs text-slate-500">Cache D1 untuk Pages. Sync ke Blogger bisa ditambah setelah OAuth siap.</div>
      </div>
  ...
  async function load(){
    ...
    const r = await api("/api/integrations/blogspot/pages?account_id="+encodeURIComponent(account_id)+"&q="+encodeURIComponent(q)+"&limit=100");
  ...
  el("btnSave").onclick = async ()=>{
    ...
    const r = await api("/api/integrations/blogspot/pages", { method:"POST", body: JSON.stringify(payload) });
    el("dbg").textContent = JSON.stringify(r,null,2);
  ...
  el("btnDel").onclick = async ()=>{
    ...
    if(!confirm("Delete cached page row?")) return;
