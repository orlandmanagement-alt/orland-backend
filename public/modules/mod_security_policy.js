export default function(Orland){
  ...
  async function load(){ return await Orland.api("/api/settings/security-policy"); }
  async function save(payload){
    return await Orland.api("/api/settings/security-policy",{ method:"POST", body: JSON.stringify(payload) });
  }
      ...
      <div class="bg-white dark:bg-darkLighter border border-slate-200 dark:border-darkBorder rounded-xl p-5">
        ...
        <div class="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div class="rounded-xl border border-slate-200 dark:border-darkBorder p-4">
            ...
            <div class="text-[10px] text-slate-500 mt-2">Enforcement logic bisa di-_wire_ di middleware berikutnya.</div>
          </div>
          ...
          <div class="rounded-xl border border-slate-200 dark:border-darkBorder p-4 lg:col-span-2">
            ...
            <div class="text-[10px] text-slate-500 mt-2">Sudah ada nosniff/no-store di middleware API, ini untuk mode enterprise berikutnya.</div>
          </div>
        </div>
