export default function(Orland){
  ...
  async function save(payload){
    return await Orland.api("/api/blogspot/pages", { method:"POST", body: JSON.stringify(payload) });
  }
  ...
  return {
    ...
    async mount(host){
      host.innerHTML = `
        <div class="space-y-4 max-w-6xl">
          <div class="flex items-center justify-between flex-wrap gap-3">
            <div>
              <div class="text-xl font-extrabold text-slate-900 dark:text-white">Blogspot Pages</div>
            ...
            <div class="flex gap-2 flex-wrap">
              ...
              <button id="btnNew" class="px-3 py-2 rounded-xl text-xs font-black bg-primary text-white">New Page</button>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div class="rounded-2xl border border-slate-200 dark:border-darkBorder bg-white dark:bg-darkLighter p-4">
              ...
              <div class="space-y-3 mt-4">
                ...
                <textarea id="content_html" rows="12" class="w-full px-4 py-3 rounded-2xl border border-slate-200 dark:border-darkBorder bg-white dark:bg-black/20 text-sm font-mono" placeholder="<div>HTML page</div>"></textarea>
                <div class="flex gap-2 flex-wrap">
          ...
          <div class="rounded-2xl border border-slate-200 dark:border-darkBorder bg-white dark:bg-darkLighter p-4">
            <div class="text-sm font-extrabold">Local Pages</div>
      ...
      async function render(){
        ...
        if(!items.length){
          q("box").innerHTML = `<div class="text-xs text-slate-500">No local pages.</div>`;
          return;
        }
      ...
      q("btnSave").onclick = async ()=>{
        ...
        q("msg").textContent = "Saved local page.";
      ...
      q("btnDelete").onclick = async ()=>{
        ...
        if(!id){
          q("msg").className = "text-sm text-red-500";
          q("msg").textContent = "Select page first.";
          return;
        }
