...
export async function onRequestPost({ request, env }){
  ...
  const endpoint = `https://www.googleapis.com/blogger/v3/blogs/${encodeURIComponent(cfg.blog_id)}/posts/${encodeURIComponent(remoteId)}`;
  ...
  await markMapDirty(env, "post", id, {
    ...
    direction: "push",
    message: "post deleted on blogger",
    payload_json: {
      remote_id: remoteId,
