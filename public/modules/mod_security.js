  ...
  return {
    ...
    async mount(host){
      host.innerHTML = `
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div class="rounded-2xl border border-slate-200 dark:border-darkBorder bg-white dark:bg-darkLighter p-4">
              <div class="text-[11px] font-bold text-slate-500">Rules fired</div>
              <div id="k_rules_fired" class="text-2xl font-black mt-1">—</div>
              <div class="text-[11px] text-slate-500 mt-1">Jumlah rule yang pernah fired dalam window.</div>
            </div>

            <div class="lg:col-span-2 rounded-2xl border border-slate-200 dark:border-darkBorder bg-white dark:bg-darkLighter p-4">
      ...
      async function render(){
        ...
        q("k_rate_limited").textContent = fmt(d.rate_limited);
        q("k_lockouts").textContent = fmt(d.lockouts);
        q("k_otp_verify_fail").textContent = fmt(d.otp_verify_fail);
        q("k_password_fail").textContent = fmt(d.password_fail);
        q("k_session_anomaly").textContent = fmt(d.session_anomaly);
        q("k_incidents_created").textContent = fmt(d.incidents_created);
        q("k_active_sessions").textContent = fmt(d.active_sessions);
        q("k_otp_requests_total").textContent = fmt(d.otp_requests_total);
        q("k_otp_consumed").textContent = fmt(d.otp_consumed);
        q("k_otp_expired").textContent = fmt(d.otp_expired);
        q("k_rules_total").textContent = fmt(d.rules_total);
        q("k_rules_enabled").textContent = fmt(d.rules_enabled);
        q("k_rules_fired").textContent = fmt(d.rules_fired);
