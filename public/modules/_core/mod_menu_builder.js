  ...
  return {
    ...
    async mount(host){
              ...
              <div>
                ...
                <div class="rounded-2xl border border-slate-200 dark:border-darkBorder p-4">
                  ...
                  <div id="roleChecks" class="space-y-2">
                    ...
                    `).join("") : `<div class="text-sm text-slate-500">No roles available.</div>`}
                  </div>
                </div>
              </div>
            </div>
      ...
      function openEditor(row = {}){
        ...
        q("btnCancelMenu").onclick = ()=>closeModal(host);
      ...
      async function load(){
        ...
        ROLES = normalizeRoles(r.data?.roles);

        renderRoleFilter();
        renderList();

        if(!ROLES.length){
          setMsg(host, "#msg", "error", "Roles not loaded.");
          return;
        }
