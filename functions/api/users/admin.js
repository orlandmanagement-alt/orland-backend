...
export async function onRequestPost({ request, env }){
  ...
  if(!email.includes("@") || password.length < 10){
    return json(400, "invalid_input", { message: "email/password" });
  }
  ...
  if(role === "super_admin" && !hasRole(a.roles, ["super_admin"])){
    return json(403, "forbidden", { message: "super_admin_only" });
  }
  ...
  if(used){
    return json(409, "conflict", { message: "email_used" });
  }
...
export async function onRequestPut({ request, env }){
  ...
  if(!user_id){
    return json(400, "invalid_input", { message: "user_id" });
  }
  ...
  if(action === "reset_password"){
    ...
    if(new_password.length < 10){
      return json(400, "invalid_input", { message: "min10" });
    }
  ...
  if(action === "set_roles"){
    const roles = Array.isArray(body.roles)
      ? body.roles.map(String).filter(Boolean)
      : [];

    if(!roles.length){
      return json(400, "invalid_input", { message: "roles" });
    }

    if(roles.includes("super_admin") && !hasRole(a.roles, ["super_admin"])){
      return json(403, "forbidden", { message: "super_admin_only" });
    }
