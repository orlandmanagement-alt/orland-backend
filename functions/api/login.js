import { json, readJson, pbkdf2Hash, timingSafeEqual, normEmail, getRolesForUser, createSession, requireEnv, cookie, sha256Base64, audit } from "../_lib.js";
...
export async function onRequestPost({ request, env }) {
  ...
  if (policy.lock_enabled && env.KV) {
    const lockKey = policy.lock_scope === "email+ip"
      ? `${email_key}:${ip_key}`
      : `${email_key}`;
  ...
  const u = await env.DB.prepare(`
    SELECT id, email_norm, display_name, status, password_hash, password_salt,
           password_iter, locked_until, lock_reason, pw_fail_count, pw_fail_last_at
    FROM users
    WHERE email_norm = ?
    LIMIT 1
  `).bind(email).first();
  ...
  if (!u) {
    if (policy.lock_enabled) {
      ...
      if (n >= policy.lock_threshold) {
        const lk = policy.lock_scope === "email+ip"
          ? `${email_key}:${ip_key}`
          : `${email_key}`;
  ...
  if (!timingSafeEqual(calc, u.password_hash)) {
    let roles = [];
    try {
      roles = await getRolesForUser(env, u.id);
    } catch {}
  ...
  if (isSuperAdmin(roles)) {
    try {
      await env.DB.prepare(
        "UPDATE users SET pw_fail_count=0, pw_fail_last_at=NULL, locked_until=NULL, lock_reason=NULL WHERE id=?"
      ).bind(u.id).run();
    } catch {}
  }
