/**
 * Orland Dashboard — shared lib (MINIMAL LOGIN MODE)
 * Fokus:
 * - JSON helpers
 * - password hash
 * - session SID cookie
 * - auth check
 * - tanpa enforcement security/rate limit/lock
 */
...
export function cookie(name, value, opt = {}) {
  ...
  // biarkan default aman, tapi tetap kompatibel untuk login biasa
...
// audit dibuat aman: best-effort, tidak pernah block login
 ...
 * - minimal, stabil, tanpa binding security tambahan
 */
export async function createSession(env, user_id, roles) {
  const now = nowSec();
  const r = Array.isArray(roles) ? roles : [];

  // dibuat simpel dan panjang dulu supaya login stabil
  let ttlMin = toNum(env.SESSION_TTL_MIN, 720); // default 12 jam
...
/* =========================================================
   Stub helpers: sengaja dibuat longgar supaya tidak memblok login
   ========================================================= */
