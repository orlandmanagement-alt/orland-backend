  ...
  return {
    ...
    async mount(host){
      ...
      <div class="bg-white dark:bg-darkLighter border border-slate-200 dark:border-darkBorder rounded-xl p-5">
        <div class="text-base font-bold">Import Data</div>
        <div class="text-xs text-slate-500 mt-1">Endpoint bisa kamu sambungkan ke job/tasks nanti (cron/manual).</div>

        <div class="mt-4 flex gap-2 flex-wrap">
          <button id="btnImport" class="px-3 py-2 rounded-xl text-xs font-bold bg-primary text-white hover:opacity-90">
            <i class="fa-solid fa-file-import mr-2"></i>Run Import
          </button>
          <button id="btnGoExport" class="px-3 py-2 rounded-xl text-xs font-bold border border-slate-200 dark:border-darkBorder hover:bg-slate-50 dark:hover:bg-white/5">
            <i class="fa-solid fa-file-export mr-2"></i>Go Export
          </button>
        </div>
      ...
      host.querySelector("#btnGoExport").onclick=()=>Orland.navigate("/data/export");
      host.querySelector("#btnImport").onclick=async ()=>{
        out.classList.remove("hidden");
        out.textContent="Calling /api/data/import (if exists)…";
        const r=await Orland.api("/api/data/import");
