...
export async function onRequestPost({ request, env }){
  ...
  const window_minutes = Math.max(1, Math.min(1440, Number(body.window_minutes||60)));
  const threshold = Math.max(1, Number(body.threshold||1));
  const severity = String(body.severity||"medium").trim();
  const cooldown_minutes = Math.max(0, Math.min(1440, Number(body.cooldown_minutes||60)));
