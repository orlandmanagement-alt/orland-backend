...
export default function(Orland){
  ...
  async function loadLocal(){ return await Orland.api("/api/blogspot/pages"); }
  async function loadRemote(){ return await Orland.api("/api/blogspot/pages?source=remote&maxResults=10"); }
  async function loadSchedules(siteId = ""){
    const q = new URLSearchParams({ limit:"200", item_kind:"page" });
  ...
  async function loadLedger(itemId, siteId = ""){
    const q = new URLSearchParams({ item_kind:"page", item_id:itemId, limit:"30" });
    if(siteId) q.set("site_id", siteId);
  ...
  <p>Isi halaman sederhana di sini.</p>
</div>`,
    contact: `<div class="space-y-4">
  <h2 class="text-2xl font-bold">Contact Us</h2>
  <p>Email: support@example.com</p>
  <p>Phone: +62 xxx xxxx</p>
</div>`,
    about: `<div class="space-y-4">
  <h2 class="text-2xl font-bold">About Us</h2>
  <p>Tulis profil perusahaan atau organisasi di sini.</p>
</div>`
  };
  ...
  function renderRemoteItems(items){
    return !items.length
      ? `<div class="text-sm text-slate-500">No remote pages.</div>`
      : items.map(x => `
  ...
  return {
    title:"Blogspot Pages",
    async mount(host){
      host.innerHTML = `
        <div class="space-y-4 max-w-6xl">
          <div class="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <div class="text-2xl font-extrabold text-slate-900 dark:text-white">Blogspot Pages</div>
              <div class="text-sm text-slate-500">Local CMS pages + remote Blogger + schedule + audit.</div>
            </div>
          ...
          <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
            <div class="xl:col-span-2 rounded-3xl border p-5">
              <div class="flex items-center justify-between gap-3 flex-wrap">
                <div class="text-xl font-extrabold">Local Pages</div>
            ...
            <div class="rounded-3xl border p-5">
              ...
              <div id="remoteBox" class="mt-4 space-y-3">
                <div class="text-sm text-slate-500">Click "Remote Preview" untuk memuat Blogger pages.</div>
              </div>
            </div>
          </div>
        </div>
      ...
      function openModal(title, body){
        q("modalTitle").textContent = title || "Page";
      ...
      function findScheduleForPage(id){
          ...
          <form id="pageForm" class="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div class="space-y-4">
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  <button type="button" class="tplBtn px-3 py-2 rounded-xl border text-xs font-bold" data-template="simple">Simple</button>
                  <button type="button" class="tplBtn px-3 py-2 rounded-xl border text-xs font-bold" data-template="contact">Contact</button>
                  <button type="button" class="tplBtn px-3 py-2 rounded-xl border text-xs font-bold" data-template="about">About</button>
                </div>
              </div>
              ...
              <div class="rounded-2xl border p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                  ...
                  <button type="button" id="btnSchedulePage" class="px-4 py-3 rounded-2xl border border-violet-200 text-violet-700 font-black text-sm">Create Schedule</button>
                </div>
              </div>
              ` : ``}
              ...
              <div class="flex gap-2 flex-wrap">
                ...
                <button type="button" id="btnCancelPage" class="px-4 py-2.5 rounded-2xl border font-black text-sm">Cancel</button>
              </div>
      ...
      function openDeleteRemote(row){
        openConfirm(
          ...
          async ()=>{
            ...
            const r = await deleteRemotePage(row.id, ACTIVE_SITE_ID);
            if(r.status !== "ok"){
      ...
      function openPublishRemote(row){
        openConfirm(
          ...
          async ()=>{
            ...
            const r = await publishPage(row.id, ACTIVE_SITE_ID);
            if(r.status !== "ok"){
      ...
      function bindModal(row = {}){
        ...
        q("btnRefreshRemotePage")?.addEventListener("click", async ()=>{
          ...
          const r = await refreshRemotePage(row.id, ACTIVE_SITE_ID);
          if(r.status !== "ok"){
            setMsg("error", "Remote refresh failed: " + r.status);
            return;
          }
          closeModal();
          setMsg("success", r.data?.remote_deleted ? "Remote page not found. Local map updated." : "Remote page refreshed.");
          await render();
        });

        q("btnSchedulePage")?.addEventListener("click", async ()=>{
          ...
          const r = await createSchedule({
            ...
            note:"scheduled from page editor"
          });
          if(r.status !== "ok"){
      ...
      function renderList(){
        const rows = filteredItems().map(x => ({ ...x, __schedule: findScheduleForPage(x.id) }));
        const pager = pagedItems(rows);

        if(!rows.length){
          q("listBox").innerHTML = `<div class="text-sm text-slate-500">No local pages.</div>`;
        ...
        q("listBox").querySelectorAll(".itemRow").forEach(btn => {
          btn.onclick = ()=>{
            ...
            if(!row) return;
            openModal("Edit Page", formHtml(row));
            bindModal(row);
          };
        });
      }
