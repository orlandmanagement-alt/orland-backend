...
export default function(Orland){
  ...
  function statusBadge(v){
    return badgeState(v, { active:"emerald", inactive:"slate" }, "slate");
  }
  ...
  return {
    ...
    async mount(host){
        ...
        <div class="space-y-4 max-w-7xl">
          <div class="flex items-center justify-between gap-3 flex-wrap">
            <div>
              ...
              <div class="text-sm text-slate-500">Widget editor final dengan site picker dan preview cepat.</div>
            </div>
            <div class="flex gap-2 flex-wrap">
              <select id="sitePicker" class="px-4 py-3 rounded-2xl border min-w-[220px]">
                <option value="">Loading site...</option>
              </select>
              <button id="btnReload" class="px-4 py-3 rounded-2xl border font-black text-sm">
          ...
          <div class="rounded-3xl border p-5">
            <div class="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div class="flex gap-2 flex-wrap">
                <select id="fSection" class="px-4 py-3 rounded-2xl border text-sm font-semibold">
                ...
                <select id="fStatus" class="px-4 py-3 rounded-2xl border text-sm font-semibold">
          ...
          <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            <div class="rounded-3xl border p-5">
              ...
              <div id="previewBox" class="mt-4 rounded-2xl border bg-slate-50 p-4 min-h-[240px] overflow-auto">
          ...
          <div class="min-h-full flex items-start lg:items-center justify-center">
            <div class="w-full max-w-6xl rounded-3xl border bg-white dark:bg-darkLighter shadow-2xl">
              <div class="px-4 lg:px-5 py-4 border-b flex items-center justify-between gap-3">
                ...
                <button id="btnModalClose" class="w-10 h-10 rounded-full border">
      ...
      function renderList(){
        if(!ITEMS.length){
          q("listBox").innerHTML = emptyState("No widgets found.");
          return;
        }

        q("listBox").innerHTML = ITEMS.map(x => `
          <button class="w-full text-left rounded-2xl border p-4 hover:bg-slate-50 itemRow ${ACTIVE && ACTIVE.id === x.id ? 'ring-2 ring-primary/40' : ''}" data-id="${esc(x.id)}">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                ...
                <div class="text-xs text-slate-500 mt-1">site: ${esc(ACTIVE_SITE_ID || "-")} • updated: ${esc(fmtTs(x.updated_at))}</div>
        ...
        q("listBox").querySelectorAll(".btnDelete").forEach(el => {
          el.onclick = (ev)=>{
            ...
            openConfirm(host, {
              ...
              confirmDesc: "This action will remove widget block from database.",
              confirmMeta: `<div class="font-black text-red-600">${esc(row.title || row.id)}</div><div class="text-xs text-slate-500 mt-2">section: ${esc(row.section || "-")} • sort: ${esc(row.sort_order)}</div>`
            });

            confirmAction = async ()=>{
              setMsg(host, "#msg", "muted", "Deleting widget...");
              const r = await saveWidget({ action:"delete", id: row.id });
              if(r.status !== "ok"){
                setMsg(host, "#msg", "error", "Delete failed: " + r.status);
                return;
              }
              closeConfirm(host);
              setMsg(host, "#msg", "success", "Widget deleted.");
      ...
      function renderPreview(){
        if(!ACTIVE){
          q("previewBox").innerHTML = emptyState("Pilih widget dari list untuk preview.");
          return;
        }
        q("previewBox").innerHTML = ACTIVE?.payload_json?.html || emptyState("No HTML payload.");
      }
          ...
          <form id="widgetForm" class="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select name="section" class="w-full px-4 py-3 rounded-2xl border text-sm font-semibold">
                ...
                <div>
                  ...
                  <input name="sort_order" type="number" value="${esc(row.sort_order ?? 0)}" class="w-full px-4 py-3 rounded-2xl border text-sm font-semibold">
                </div>
              ...
              <div>
                ...
                <div class="flex gap-2 flex-wrap">
                  <button type="button" class="tplBtn px-3 py-2 rounded-xl border text-xs font-black" data-template="hero">Hero</button>
                  <button type="button" class="tplBtn px-3 py-2 rounded-xl border text-xs font-black" data-template="card_grid">Card Grid</button>
                  <button type="button" class="tplBtn px-3 py-2 rounded-xl border text-xs font-black" data-template="notice">Notice</button>
                  <button type="button" class="tplBtn px-3 py-2 rounded-xl border text-xs font-black" data-template="html_block">HTML Block</button>
                </div>
              </div>

              <div class="flex gap-2 flex-wrap">
                <button type="submit" class="px-4 py-2.5 rounded-2xl bg-primary text-white font-black text-sm">Save</button>
                <button type="button" id="btnCancelWidget" class="px-4 py-2.5 rounded-2xl border font-black text-sm">Cancel</button>
              </div>
            </div>
            ...
            <div class="space-y-4">
              <div>
                ...
                <textarea id="htmlPayload" rows="18" class="w-full px-4 py-3 rounded-2xl border text-sm font-mono">${esc(payload.html || "")}</textarea>
              </div>

              <div class="rounded-2xl border p-3">
                <div class="text-xs font-black text-slate-500 mb-2">LIVE PREVIEW</div>
                <div id="htmlPreview" class="rounded-2xl bg-slate-50 p-4 min-h-[220px] overflow-auto"></div>
              </div>
            </div>
          </form>
        `;
      ...
      function openEditor(row = {}){
        openModal(host, {
          modalTitle: row.id ? "Edit Widget" : "Create Widget",
        ...
        function renderHtml(){
          preview.innerHTML = htmlEl.value || emptyState("Empty preview.");
        }

        renderHtml();
        htmlEl.addEventListener("input", renderHtml);

        q("btnCancelWidget").onclick = ()=>closeModal(host);
        ...
        form.onsubmit = async (ev)=>{
          ...
          if(!payload.title){
            setMsg(host, "#msg", "error", "Title required.");
            return;
          }

          setMsg(host, "#msg", "muted", "Saving widget...");
          const r = await saveWidget(payload);
          if(r.status !== "ok"){
            setMsg(host, "#msg", "error", "Save failed: " + r.status);
            return;
          }

          closeModal(host);
          setMsg(host, "#msg", "success", "Widget saved.");
          await render();
        };
      }
      ...
      async function render(){
        ...
        setMsg(host, "#msg", "muted", "Loading widgets...");
