...
export default function BlogspotPages(ctx){
  const { api, toast, setBreadcrumb } = ctx;
  const el=document.createElement("div");
  el.innerHTML=`
    <div class="flex items-center justify-between gap-3">
      <div>
        <h2 class="text-xl font-bold text-slate-900 dark:text-white">Static Pages</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Stored in D1 (cms_items kind=page)</p>
      </div>
  ...
  async function load(){
    ...
    const url="/api/integrations/blogspot/pages?limit=80"+(q?("&q="+encodeURIComponent(q)):"")+(st?("&status="+encodeURIComponent(st)):"");
  ...
  async function create(){
    ...
    const r=await api("/api/integrations/blogspot/pages",{method:"POST",body:JSON.stringify({title,slug,status,content_html})});
    el.querySelector("#out").textContent=JSON.stringify(r,null,2);
  ...
  async function edit(id){
    ...
    const r=await api("/api/integrations/blogspot/pages",{method:"PUT",body:JSON.stringify(payload)});
    el.querySelector("#out").textContent=JSON.stringify(r,null,2);
    toast(r.status,r.status==="ok"?"success":"error");
    if(r.status==="ok") load();
  }
  ...
  async function del(id){
    ...
    const r=await api("/api/integrations/blogspot/pages?id="+encodeURIComponent(id),{method:"DELETE"});
    el.querySelector("#out").textContent=JSON.stringify(r,null,2);
    toast(r.status,r.status==="ok"?"success":"error");
    if(r.status==="ok") load();
  }
  ...
  return {
    mount(host){
      setBreadcrumb("/ integrations / blogspot / pages");
