...
export async function onRequestGet({ request, env }){
  ...
  const maxResults = Math.max(1, Math.min(50, Number(urlObj.searchParams.get("maxResults") || "10")));
  const pageToken = String(urlObj.searchParams.get("pageToken") || "").trim();

  const url = bloggerUrl(`blogs/${cfg.blog_id}/posts`, {
    key: cfg.api_key,
    fetchBodies: "true",
    status: "live",
    maxResults,
