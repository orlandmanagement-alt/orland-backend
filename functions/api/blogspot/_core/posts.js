...
export async function onRequestGet({ request, env }){
  ...
  const items = (rows.results || []).map(x => ({
    ...x,
    labels_json: safeJsonParse(x.labels_json, []),
...
export async function onRequestPost({ request, env }){
  ...
  if(action === "delete"){
    ...
    const row = await env.DB.prepare(`SELECT id, title, slug FROM cms_posts WHERE id=? LIMIT 1`).bind(id).first();
    if(!row) return json(404, "not_found", { error:"post_not_found" });

    await env.DB.prepare(`DELETE FROM cms_posts WHERE id=?`).bind(id).run();
  ...
  const id = mode === "create" ? (s(body.id) || makeId("post")) : s(body.id);
  if(!id) return json(400, "invalid_input", { error:"id_required" });
  ...
  const content_text = s(body.content_text || "");
  const labels = Array.isArray(body.labels_json) ? body.labels_json.map(x => String(x || "").trim()).filter(Boolean) : [];
  ...
  if(mode === "update"){
    const ex = await env.DB.prepare(`SELECT id FROM cms_posts WHERE id=? LIMIT 1`).bind(id).first();
    if(!ex) return json(404, "not_found", { error:"post_not_found" });

    await env.DB.prepare(`
      UPDATE cms_posts
      SET provider=?, account_id=?, external_id=?, blog_id=?, title=?, slug=?, status=?, url=?,
          content_html=?, content_text=?, labels_json=?, meta_json=?, published_at=?, updated_at=?
      WHERE id=?
    `).bind(
      "blogspot",
      cfg.blog_id || "local",
