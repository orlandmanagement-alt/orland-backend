...
export async function onRequestGet({ request, env }){
  ...
  const r = await env.DB.prepare(`
    SELECT id,title,slug,status,tags_json,created_at,updated_at,created_by_user_id,updated_by_user_id
    FROM cms_posts
  ...
  return json(200,"ok",{ posts: r.results || [] });
}
...
export async function onRequestPost({ request, env }){
  ...
  await env.DB.prepare(`
    INSERT INTO cms_posts (id,title,slug,status,tags_json,body,meta_json,created_by_user_id,updated_by_user_id,created_at,updated_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)
