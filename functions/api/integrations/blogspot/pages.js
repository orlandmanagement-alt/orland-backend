...
export async function onRequestPost({ request, env }){
  ...
  await env.DB.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).bind(
    id,PROVIDER,ACCOUNT_ID,KIND,null,title,slug,content_html,JSON.stringify(meta||{}),
    (status==="published"?"published":"draft"), published_at, now, now
  ).run();

  await audit(env,{ actor_user_id:a.uid, action:"blogspot.pages.create", route:"POST /api/integrations/blogspot/pages", http_status:200, meta:{ id } });
...
export async function onRequestPut({ request, env }){
  ...
  await audit(env,{ actor_user_id:a.uid, action:"blogspot.pages.update", route:"PUT /api/integrations/blogspot/pages", http_status:200, meta:{ id } });
...
export async function onRequestDelete({ request, env }){
  ...
  await audit(env,{ actor_user_id:a.uid, action:"blogspot.pages.delete", route:"DELETE /api/integrations/blogspot/pages", http_status:200, meta:{ id } });
